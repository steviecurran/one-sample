...
form = 'd' #str(input("Data format, csv or  dat [c/d]? "))
if form != "d":
    os.system("ls *.csv")
else:
    os.system("ls *.dat")
...
hq = str(input("Is there a header [y/n]? " ))
...
def histo(dbs):
    ...
    plt.tight_layout()
    outfile = '%s-histo.eps' %(infile) # HAS TO BE HERE, REMOVED dbs SO OVERWRITTEN
...
def z_bit(con):
    ...
    plt.fill_between(xarr,yarr,0, color='r')

    xlabel = xf - (xf-xi)/1.7; ylabel = 0.38;
    text = "p = %1.4f%% $\Rightarrow$ Z = %1.2f$\sigma$" % (con,Z)
    ax.text(xlabel,ylabel,text,fontsize = 12, horizontalalignment='left',verticalalignment='top') 
    plt.xlabel('Z-value [$\sigma$]'); plt.ylabel('Probability')

    plt.tight_layout()
    outfile = 'CI-con=%1.5f.eps' %(con)
    #plt.savefig(outfile, format='eps'); print('Written to %s' %(outfile))
    #plt.show();
    ########################################################################
    
    CI = Z*SE
    print("For %1.2f%% confidence, z = %1.3f, giving mean diff of %1.2f +/- %1.2f (%1.2f to %1.2f)"
